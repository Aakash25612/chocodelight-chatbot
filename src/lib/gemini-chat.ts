...
- CRITICAL — "sauda" / "pending sauda" / "X tell his pending sauda" is ALWAYS get_pending_sauda (Locked orders, quantity − quantityShipped > 0). Pass query=customer name when named. NEVER use get_customer_statement, get_outstanding_receivables, get_receivables_aging, or search_sales_orders for sauda. Sauda ≠ outstanding balance / open invoices. Reply in English. ALWAYS present quantities as **metric tons (MT)** using pendingQuantityMT / totalPendingQuantityMT / quantityMT fields — never bags, pcs, or kg as the primary quantity.
